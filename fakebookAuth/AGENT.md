...
- Access token format: library-issued RS256 JWT with `kid`; HS256 is optional and migration-only.
...
`Jwt:PrivateKeyBase64` is a PKCS#8 RSA private key of at least 2048 bits. Only Auth receives it.
...
Access tokens are RS256 JWTs generated by `TokenService` through the standard identity-model library.
